...
#include "pmns_data.h"
#include "pmns_arith_ops.c"
#include "pmns_useful_functs.c"


//~ Compilation command: gcc -Wall -O3 main__nb_cycles.c -o cmain -lgmp -lcrypto
//~ Execution command  : ./cmain

//~ WARNING: change appropriately 'pa' and 'pb' type (lines 40 and 41) to avoid a lot of warning and potential errors
...
int main(void){
	...
	mpz_init (R2);
	init_data(); // <---- for PMNS and the module (modul_p)
	...
	for(int i=0;i<NTEST;i++){
		...
		from_int_to_pmns(pa[i], A[i]);
		from_int_to_pmns(pb[i], B[i]);
	...
	for(int i=0;i<NSAMPLES;i++){
		...
		for(int j=0;j<NTEST;j++){
			...
			from_int_to_pmns(pa[j], A[j]);
			from_int_to_pmns(pb[j], B[j]);
